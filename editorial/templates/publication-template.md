# Publication Title

## Summary

Two to four sentences explaining the publication and why it matters. Write this so it can also stand alone in search results, GitBook snippets, or link previews.

## Publication Details

- Publisher:
- Published date:
- Region:
- Content type:
- Reviewed:

## Key Takeaways

- Takeaway 1
- Takeaway 2

## Cover Or Supporting Image

```md
![Publication cover](../media/publications/publication-slug/cover.png)
```

## Video

Use when there is a launch talk, interview, or report walkthrough.

```md
{% embed url="https://www.youtube.com/watch?v=example" %}
```

## Full Report

[Open report](https://example.com)

## Next Step

Use when the publication naturally supports a UBI Global next action such as academy, membership, benchmark participation, or related research.

- Internal next step:
- UBI Global next step:
